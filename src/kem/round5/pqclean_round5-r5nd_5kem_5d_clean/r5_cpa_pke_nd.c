...
static void PQCLEAN_ROUND5R5ND_5KEM_5D_pack_q_p(uint8_t *pv, const modq_t *vq, const modq_t rounding_constant) {
    #if (PARAMS_P_BITS == 8)
    size_t i;
...
static void PQCLEAN_ROUND5R5ND_5KEM_5D_unpack_p(modp_t *vp, const uint8_t *pv) {
    #if (PARAMS_P_BITS == 8)
...
int PQCLEAN_ROUND5R5ND_5KEM_5D_r5_cpa_pke_keygen(uint8_t *pk, uint8_t *sk) {
    ...
    // A from sigma
    PQCLEAN_ROUND5R5ND_5KEM_5D_create_A_random(A, pk);

    randombytes(sk, PARAMS_KAPPA_BYTES); // secret key -- Random S
    PQCLEAN_ROUND5R5ND_5KEM_5D_create_secret_vector(S_idx, sk);

    PQCLEAN_ROUND5R5ND_5KEM_5D_ringmul_q(B, A, S_idx); // B = A * S

    // Compress B q_bits -> p_bits, pk = sigma | B
    PQCLEAN_ROUND5R5ND_5KEM_5D_pack_q_p(pk + PARAMS_KAPPA_BYTES, B, PARAMS_H1);
...
int PQCLEAN_ROUND5R5ND_5KEM_5D_r5_cpa_pke_encrypt(uint8_t *ct, const uint8_t *pk, const uint8_t *m, const uint8_t *rho) {
    ...
    // unpack public key
    PQCLEAN_ROUND5R5ND_5KEM_5D_unpack_p(B, pk + PARAMS_KAPPA_BYTES);
    ...
    // Create R
    PQCLEAN_ROUND5R5ND_5KEM_5D_create_secret_vector(R_idx, rho);

    PQCLEAN_ROUND5R5ND_5KEM_5D_ringmul_q(U_T, A, R_idx); // U^T == U = A^T * R == A * R (mod q)
    PQCLEAN_ROUND5R5ND_5KEM_5D_ringmul_p(X, B, R_idx); // X = B^T * R == B * R (mod p)
    ...
    PQCLEAN_ROUND5R5ND_5KEM_5D_pack_q_p(ct, U_T, PARAMS_H2); // ct = U^T | v
...
int PQCLEAN_ROUND5R5ND_5KEM_5D_r5_cpa_pke_decrypt(uint8_t *m, const uint8_t *sk, const uint8_t *ct) {
    ...
    PQCLEAN_ROUND5R5ND_5KEM_5D_unpack_p(U_T, ct); // ct = U^T | v
    ...
    PQCLEAN_ROUND5R5ND_5KEM_5D_ringmul_p(X_prime, U_T, S_idx); // X' = S^T * U == U^T * S (mod p)
