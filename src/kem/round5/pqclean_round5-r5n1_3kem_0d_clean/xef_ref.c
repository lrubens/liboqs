...
const unsigned PQCLEAN_ROUND5R5N1_3KEM_0D_CLEAN_xef_reg[5][3][10] = {
    {
        { 11, 13 }, // XE1-24
        { 13, 15 }, // XE1-28
        { 16, 16 }  // XE1-32
...
size_t PQCLEAN_ROUND5R5N1_3KEM_0D_CLEAN_xef_compute(void *block, size_t len, unsigned f) {
    ...
    for (i = 0; i < len; i++) {
        ...
        for (; j < 2 * f; j++) {
            r[j] ^= x << (bit % PQCLEAN_ROUND5R5N1_3KEM_0D_CLEAN_xef_reg[f - 1][pl][j]);
    ...
    for (i = 0; i < 2 * f; i++) {
        ...
        l = PQCLEAN_ROUND5R5N1_3KEM_0D_CLEAN_xef_reg[f - 1][pl][i];      // len
        x = r[i];
        x ^= x >> l;
...
size_t PQCLEAN_ROUND5R5N1_3KEM_0D_CLEAN_xef_fixerr(void *block, size_t len, unsigned f) {
    ...
    for (i = 0; i < (len << 3); i++) {
        ...
        for (; j < 2 * f; j++) {
            th += (unsigned) (r[j] >> (i %  PQCLEAN_ROUND5R5N1_3KEM_0D_CLEAN_xef_reg[f - 1][pl][j])) & 1;
        }
        // if th > f
