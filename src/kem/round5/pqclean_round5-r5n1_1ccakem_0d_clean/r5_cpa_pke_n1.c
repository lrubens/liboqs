...
#endif
#define PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_xef_compute(block, len, f) XEF(compute, block, len, f)
#define PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_xef_fixerr(block, len, f) XEF(fixerr, block, len, f)
...
static int PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_create_A_permutation(uint32_t A_permutation[PARAMS_D], const unsigned char *sigma) {
    /* Compute the permutation */
    uint16_t rnd;
    r5_xof_ctx_t ctx;
    PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_r5_xof_s_input(&ctx, sigma, PARAMS_KAPPA_BYTES, permutation_customization, sizeof(permutation_customization));

    for (uint32_t i = 0; i < PARAMS_D; ++i) {
        do {
            PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_r5_xof_squeeze(&ctx, &rnd, sizeof(rnd));
...
static int PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_create_A_permutation(uint16_t A_permutation[PARAMS_D], const unsigned char *sigma) {
    /* Compute the permutation */
    uint16_t rnd = 0;
    uint32_t i;
...
static void PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_pack_q_p_m_bar(uint8_t *pv, const modq_t *vq, const modq_t rounding_constant) {
    #if (PARAMS_P_BITS == 8)
    size_t i;
...
static void PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_pack_q_p_n_bar(uint8_t *pv, const modq_t *vq, const modq_t rounding_constant) {
    #if (PARAMS_P_BITS == 8)
    size_t i;
...
static void PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_unpack_p_m_bar(modp_t *vp, const uint8_t *pv) {
    #if (PARAMS_P_BITS == 8)
...
static void PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_unpack_p_n_bar(modp_t *vp, const uint8_t *pv) {
    #if (PARAMS_P_BITS == 8)
...
int PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_r5_cpa_pke_keygen(uint8_t *pk, uint8_t *sk) {
    ...
    PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_print_hex("r5_cpa_pke_keygen: sigma", pk, PARAMS_KAPPA_BYTES, 1);
    #endif
...
#define A_element(r,c) A_random[A_permutation[r] + (uint32_t) c]
    ...
    PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_create_secret_matrix_s_t(S_T, sk);
    #if PARAMS_TAU == 0
    PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_matmul_as_q(B, A_matrix, S_T); // B = A * S
    #else
    PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_matmul_as_q(B, A_matrix, A_permutation, S_T); // B = A * S
    #endif


    // Compress B q_bits -> p_bits, pk = sigma | B
    PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_pack_q_p_n_bar(pk + PARAMS_KAPPA_BYTES, &B[0][0], PARAMS_H1);
    return 0;
}

int PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_r5_cpa_pke_encrypt(uint8_t *ct, const uint8_t *pk, const uint8_t *m, const uint8_t *rho) {
    size_t i, j;
    #ifdef CM_CACHE
    ...
    // unpack public key
    PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_unpack_p_n_bar(&B[0][0], pk + PARAMS_KAPPA_BYTES);
...
#define A_element(r,c) A_random[A_permutation[r] + (uint32_t) c]
    ...
    #if (PARAMS_XE != 0)
    PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_xef_compute(m1, PARAMS_KAPPA_BYTES, PARAMS_F);
    #endif

    // Create R
    PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_create_secret_matrix_r_t(R_T, rho);

    #if PARAMS_TAU == 0
    PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_matmul_rta_q(U_T, A_matrix, R_T); // U^T = (R^T x A)^T   (mod q)
    #else
    PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_matmul_rta_q(U_T, A_matrix, A_permutation, R_T); // U^T = (R^T x A)^T   (mod q)
    #endif
    PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_matmul_btr_p(X, B, R_T); // X = R^T x B   (mod p)

    #ifdef NIST_KAT_GENERATION
    PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_print_hex("r5_cpa_pke_encrypt: rho", rho, PARAMS_KAPPA_BYTES, 1);
    PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_print_hex("r5_cpa_pke_encrypt: sigma", pk, PARAMS_KAPPA_BYTES, 1);
    PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_print_hex("r5_cpa_pke_encrypt: m1", m1, BITS_TO_BYTES(PARAMS_MU * PARAMS_B_BITS), 1);
    #endif

    PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_pack_q_p_m_bar(ct, &U_T[0][0], PARAMS_H2); // ct = U^T | v
...
int PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_r5_cpa_pke_decrypt(uint8_t *m, const uint8_t *sk, const uint8_t *ct) {
    size_t i, j;
    #ifdef CM_CACHE
    int16_t S_T[PARAMS_N_BAR][PARAMS_D];
    #else
    ...
    PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_unpack_p_m_bar((modp_t *) U_T, ct); // ct = U^T | v
    ...
    // X' = S^T * U (mod p)
    PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_matmul_stu_p(X_prime, U_T, S_T);
    ...
    PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_xef_compute(m1, PARAMS_KAPPA_BYTES, PARAMS_F);
    PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_xef_fixerr(m1, PARAMS_KAPPA_BYTES, PARAMS_F);
    #endif
    memcpy(m, m1, PARAMS_KAPPA_BYTES);

    #ifdef NIST_KAT_GENERATION
    PQCLEAN_ROUND5R5N1_1CCAKEM_0D_CLEAN_print_hex("r5_cpa_pke_decrypt: m", m, PARAMS_KAPPA_BYTES, 1);
    #endif
