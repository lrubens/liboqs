...
int PQCLEAN_ROUND5R5N1_3CCAKEM_0D_CLEAN_round5_dem(unsigned char *c2, unsigned long long *c2_len, const unsigned char *key, const unsigned char *m, const unsigned long long m_len) {
    int result = 1;
    int len;
    int c2length;
    ...
    PQCLEAN_ROUND5R5N1_3CCAKEM_0D_CLEAN_hash(final_key_iv, (size_t) (PARAMS_KAPPA_BYTES + 12), key, PARAMS_KAPPA_BYTES, PARAMS_KAPPA_BYTES);
...
int PQCLEAN_ROUND5R5N1_3CCAKEM_0D_CLEAN_round5_dem_inverse(unsigned char *m, unsigned long long *m_len, const unsigned char *key, const unsigned char *c2, const unsigned long long c2_len) {
    int result = 1;
    int len;
    int m_length;
    EVP_CIPHER_CTX *ctx = NULL;
    ...
    if ((diff >= 0 && diff < (ptrdiff_t) c2_len_no_tag) || (diff < 0 && diff > -((ptrdiff_t) c2_len_no_tag))) {
        ...
        tmp_m = PQCLEAN_ROUND5R5N1_3CCAKEM_0D_CLEAN_checked_malloc(c2_len_no_tag);
        // tmp_m[c2_len_no_tag];
    }
