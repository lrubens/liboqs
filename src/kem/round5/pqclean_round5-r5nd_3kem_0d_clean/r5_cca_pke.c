...
int PQCLEAN_ROUND5R5ND_3KEM_0D_CLEAN_r5_cca_pke_keygen(unsigned char *pk, unsigned char *sk) {
    return PQCLEAN_ROUND5R5ND_3KEM_0D_CLEAN_r5_cca_kem_keygen(pk, sk);
}
...
int PQCLEAN_ROUND5R5ND_3KEM_0D_CLEAN_r5_cca_pke_encrypt(unsigned char *ct, unsigned long long *ct_len, const unsigned char *m, const unsigned long long m_len, const unsigned char *pk) {
    ...
    /* Determine c1 and k */
    PQCLEAN_ROUND5R5ND_3KEM_0D_CLEAN_r5_cca_kem_encapsulate(c1, k, pk);
    ...
    *ct_len = c1_len;
    ...
    if (PQCLEAN_ROUND5R5ND_3KEM_0D_CLEAN_round5_dem(ct + c1_len, &c2_len, k, m, m_len)) {
        goto done_encrypt;
    }
    *ct_len += c2_len;
...
int PQCLEAN_ROUND5R5ND_3KEM_0D_CLEAN_r5_cca_pke_decrypt(unsigned char *m, unsigned long long *m_len, const unsigned char *ct, unsigned long long ct_len, const unsigned char *sk) {
    int result = -1;
    unsigned char k[PARAMS_KAPPA_BYTES];
    ...
    /* Determine k */
    PQCLEAN_ROUND5R5ND_3KEM_0D_CLEAN_r5_cca_kem_decapsulate(k, c1, sk);

    /* Apply DEM-inverse to get m */
    if (PQCLEAN_ROUND5R5ND_3KEM_0D_CLEAN_round5_dem_inverse(m, m_len, k, c2, c2_len)) {
        goto done_decrypt;
    }
