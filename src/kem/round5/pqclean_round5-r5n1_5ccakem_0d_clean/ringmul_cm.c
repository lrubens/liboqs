...
void PQCLEAN_ROUND5R5N1_5CCAKEM_0D_CLEAN_create_secret_vector(uint16_t idx[PARAMS_H / 2][2], const uint8_t *seed) {
    size_t i;
    uint16_t x;
    ...
    memset(v, 0, sizeof (v));
    PQCLEAN_ROUND5R5N1_5CCAKEM_0D_CLEAN_r5_xof_input(&ctx, seed, PARAMS_KAPPA_BYTES);    

    for (i = 0; i < PARAMS_H; i++) {
        do {
            do {
                PQCLEAN_ROUND5R5N1_5CCAKEM_0D_CLEAN_r5_xof_squeeze(&ctx, &x, sizeof(x));
            } while (x >= PARAMS_RS_LIM);
            x /= PARAMS_RS_DIV;
        } while (PQCLEAN_ROUND5R5N1_5CCAKEM_0D_CLEAN_probe_cm(v, x));
...
void PQCLEAN_ROUND5R5N1_5CCAKEM_0D_CLEAN_ringmul_q(modq_t d[PARAMS_ND], modq_t a[PARAMS_ND], uint16_t idx[PARAMS_H / 2][2]) {
    size_t i, j, k;
...
void PQCLEAN_ROUND5R5N1_5CCAKEM_0D_CLEAN_ringmul_p(modp_t d[PARAMS_MU], modp_t a[PARAMS_ND], uint16_t idx[PARAMS_H / 2][2]) {
    size_t i, j, k;
