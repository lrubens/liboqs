            ...
            do {
                do {
                    if (y == &base[PARAMS_H]) {
                        ...
                        PQCLEAN_ROUND5R5N1_5CCAKEM_0D_CLEAN_r5_xof_squeeze(&ctx, base, sizeof(base));
                        x = LITTLE_ENDIAN16(x);
                    }
                    x = *y++;
                } while (x >= PARAMS_RS_LIM);
                x /= PARAMS_RS_DIV;
            } while (PQCLEAN_ROUND5R5N1_5CCAKEM_0D_CLEAN_probe_cm_even(v, x));
            do {
                do {
                    if (y == &base[PARAMS_H]) {
                        y = base;
                ...
                x /= PARAMS_RS_DIV;
            } while (PQCLEAN_ROUND5R5N1_5CCAKEM_0D_CLEAN_probe_cm_odd(v, x));
        }
