...
void PQCLEAN_ROUND5R5N1_1KEM_0D_CLEAN_create_secret_vector(uint16_t idx[PARAMS_H / 2][2], const uint8_t *seed) {
    size_t i;
    uint16_t x;
    ...
    for (i = 0; i < PARAMS_H; i++) {
        do {
            ...
            x /= PARAMS_RS_DIV;
        } while (PQCLEAN_ROUND5R5N1_1KEM_0D_CLEAN_probe_cm(v, x));
...
void PQCLEAN_ROUND5R5N1_1KEM_0D_CLEAN_ringmul_q(modq_t d[PARAMS_ND], modq_t a[PARAMS_ND], uint16_t idx[PARAMS_H / 2][2]) {
    size_t i, j, k;
...
void PQCLEAN_ROUND5R5N1_1KEM_0D_CLEAN_ringmul_p(modp_t d[PARAMS_MU], modp_t a[PARAMS_ND], uint16_t idx[PARAMS_H / 2][2]) {
    size_t i, j, k;
