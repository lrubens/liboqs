...
int PQCLEAN_ROUND5R5N1_5KEM_0D_r5_cca_kem_keygen(uint8_t *pk, uint8_t *sk) {
    uint8_t y[PARAMS_KAPPA_BYTES];

    /* Generate the base key pair */
    PQCLEAN_ROUND5R5N1_5KEM_0D_r5_cpa_pke_keygen(pk, sk);
...
int PQCLEAN_ROUND5R5N1_5KEM_0D_r5_cca_kem_encapsulate(uint8_t *ct, uint8_t *k, const uint8_t *pk) {
    ...
    PQCLEAN_ROUND5R5N1_5KEM_0D_hash((uint8_t *) L_g_rho, 3 * PARAMS_KAPPA_BYTES, hash_in, PARAMS_KAPPA_BYTES + PARAMS_PK_SIZE, PARAMS_KAPPA_BYTES);
    ...
    /* Encrypt  */
    PQCLEAN_ROUND5R5N1_5KEM_0D_r5_cpa_pke_encrypt(ct, pk, m, L_g_rho[2]); // m: ct = (U,v)
    ...
    PQCLEAN_ROUND5R5N1_5KEM_0D_hash(k, PARAMS_KAPPA_BYTES, hash_in, PARAMS_KAPPA_BYTES + PARAMS_CT_SIZE + PARAMS_KAPPA_BYTES, PARAMS_KAPPA_BYTES);
...
int PQCLEAN_ROUND5R5N1_5KEM_0D_r5_cca_kem_decapsulate(uint8_t *k, const uint8_t *ct, const uint8_t *sk) {
    uint8_t hash_in[PARAMS_KAPPA_BYTES + (PARAMS_CT_SIZE + PARAMS_KAPPA_BYTES > PARAMS_PK_SIZE ? PARAMS_CT_SIZE + PARAMS_KAPPA_BYTES : PARAMS_PK_SIZE)];
    ...
    PQCLEAN_ROUND5R5N1_5KEM_0D_r5_cpa_pke_decrypt(m_prime, sk, ct); // r5_cpa_pke_decrypt m'
    ...
    PQCLEAN_ROUND5R5N1_5KEM_0D_hash((uint8_t *) L_g_rho_prime, 3 * PARAMS_KAPPA_BYTES, hash_in, PARAMS_KAPPA_BYTES + PARAMS_PK_SIZE, PARAMS_KAPPA_BYTES);
    ...
    // Encrypt m: ct' = (U',v')
    PQCLEAN_ROUND5R5N1_5KEM_0D_r5_cpa_pke_encrypt(ct_prime, sk + PARAMS_KAPPA_BYTES + PARAMS_KAPPA_BYTES, m_prime, L_g_rho_prime[2]);
